    ...
    subsections: [
      {
        ...
        asIsToBe: {
          asIs: '신분증·혼인관계증명·소득증명·자격증 4종의 서류를 업로드하는 인증 플로우에서 각 단계의 진행 상태 관리가 없어, 사용자가 미완료 상태에서 다음 단계로 이동하거나 인증 전 기능에 접근할 수 있었습니다.',
          toBe: '단계별 인증 상태값을 전역에서 관리하고, 완료 여부에 따라 다음 단계 접근을 제어하는 UX를 설계 및 구현했습니다. 이탈 후 재진입 시 이전 진행 상태를 복원하는 로직을 추가해 중간 이탈로 인한 재시작 불편을 해소했습니다.',
        },
      },
      {
      ...
      },
      {
        title: 'NICE 본인인증 연동',
        asIsToBe: {
          asIs: '외부 인증 서버 특성상 React에서 직접 POST 요청이 불가하여 백엔드에서 세션 토큰을 발급받아도 인증 서버로 데이터를 전달할 방법이 없었습니다.',
          toBe: "document.createElement('form')으로 동적 폼을 생성해 NICE 서버에 submit하는 방식으로 해결했습니다. returnUrl 파라미터를 통해 인증 완료 후 아이디찾기·비밀번호재설정 페이지로 각각 분기 처리했습니다.",
        },
      },
    ],
    retrospective:
    ...
    subsections: [
      {
        ...
        asIsToBe: {
          ...
          toBe: '셀러별 상품금액 · 배송비 · 할인금액을 각각 산출한 뒤 합산하는 책임 분리 구조로 재설계했습니다. 렌더링마다 반복 계산되는 비용을 useMemo로 메모이제이션하여 불필요한 연산을 줄이고, 조건이 추가되어도 해당 단위만 수정하면 되는 구조로 정리했습니다.',
        },
      },
      {
